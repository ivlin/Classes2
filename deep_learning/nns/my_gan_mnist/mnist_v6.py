...
def generator(x,reuse=False,weights=None):
    if weights==None:
        ...
        with tf.variable_scope("GAN/Generator",reuse=reuse):
            ...
            '''
            layers=[]
            layers.append({'w':tf.Variable(tf.random_normal([int(100/4*15),100],stddev=0.1),name="h1"),
                'b':tf.Variable(tf.random_normal([1,100],stddev=0.0),name="h1_bias")})
            layers.append({'w':tf.Variable(tf.random_normal([100,256],stddev=0.1),name="h2"),
                'b':tf.Variable(tf.random_normal([1,256],stddev=0.0),name="h2_bias"),
                'activation':tf.nn.relu})
            layers.append({'w':tf.Variable(tf.random_normal([256,512],stddev=0.1),name="h3"),
                'b':tf.Variable(tf.random_normal([1,512],stddev=0.0),name="h3_bias"),
                'activation':tf.nn.relu})
            layers.append({'w':tf.Variable(tf.random_normal([512,1024],stddev=0.1),name="h4"),
